...
fn solve(input: &str) -> u64 {
    ...
    let map_2 = generate_map(&instructions);
    ...
    let result = find_relevant_wires(&map_2, "z45", 0);
    println!("{result}");
    0
    ...
    // }
    // println!("{:b}", result);
    // 64 - result.leading_zeros() as u64
}

fn generate_map(instructions: &Vec<[&str; 4]>) -> HashMap<String, [String; 3]> {
    let mut map = HashMap::new();
    for [a, operation, b, target] in instructions {
        map.insert(
            target.to_string(),
            [a.to_string(), operation.to_string(), b.to_string()],
        );
    }
    map
}

fn find_relevant_wires(
    instructions: &HashMap<String, [String; 3]>,
    target: &str,
    mut i: i32,
) -> i32 {
    if let Some([a, operation, b]) = instructions.get(target) {
        println!("{a} {operation} {b}");
        find_relevant_wires(instructions, a, i + 1);
        find_relevant_wires(instructions, b, i + 1);
    }
    i
}
